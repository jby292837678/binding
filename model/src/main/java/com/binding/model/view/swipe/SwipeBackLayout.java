package com.binding.model.view.swipe;
...
import com.binding.model.view.Util;
...
public class SwipeBackLayout extends ViewGroup {
    ...
    private OnSwipeFinishedListener onSwipeFinishedListener;
    ...
    public SwipeBackLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        setOnSwipeFinishedListener((mView, isEnd) -> {if(isEnd)finish();});
        setOnSwipeBackListener((mView, swipeBackFraction1) -> this.setAlpha(1-swipeBackFraction1));
//        setBackgroundColor(App.getColor(R.color.windowBackground));
        init(context, attrs);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 0) {
            ...
            if(mDragContentView.getBackground() == null) mDragContentView.setBackgroundColor(Color.WHITE);
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (onSwipeBackListener != null) {
                onSwipeBackListener.onViewPositionChanged(mDragContentView, swipeBackFraction);
            }
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            touchedEdge = ViewDragHelper.INVALID_POINTER;
            boolean isBackToEnd = backJudgeBySpeed(xvel, yvel);
            if (isBackToEnd) {
                switch (mDirectionMode) {
                    case FROM_LEFT:
        ...
        public void onViewDragStateChanged(int state) {
            ...
            if (state == ViewDragHelper.STATE_IDLE) {
                if (onSwipeFinishedListener != null) {
                    if (swipeBackFraction == 0) {
                        onSwipeFinishedListener.onViewSwipeFinished(mDragContentView, false);
                    } else if (swipeBackFraction == 1) {
                        onSwipeFinishedListener.onViewSwipeFinished(mDragContentView, true);
                    }
                }
            }
        }
    ...
    public void setOnSwipeFinishedListener(OnSwipeFinishedListener onSwipeFinishedListener) {
        this.onSwipeFinishedListener = onSwipeFinishedListener;
    }

    public interface OnSwipeBackListener {
        void onViewPositionChanged(View mView, float swipeBackFraction);
    }

    public interface OnSwipeFinishedListener{
